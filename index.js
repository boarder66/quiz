...
function generateAnswer(){
    ...
    for (let i = 0; i < answers.length; i++){
        choices += `<input type="radio" value="${i}" id="${i}" name="answer">
        <label for="${i}" class="rightAnswer">${answers[i]}</label>`;
    };
    return choices;
...
function ifAnswerIsCorrect(){
    ...
    $("#photo-correct").fadeIn(2000);
     //click to remove img and move to next question
};

function ifAnswerIsWrong(){
    totalQuestionsAnswered++;
    //update show-right-answer
    $('#show-right-answer').text(QUESTIONANSWER[questionNumber].answers[QUESTIONANSWER[questionNumber].correctAnswer]);
